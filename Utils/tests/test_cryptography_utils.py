...
from Utils.cryptography import derive_256_bit_salt_and_key, encrypt_aes_256_gcm, decrypt_aes_256_gcm


class CryptographyUtilsTests(unittest.TestCase):
    def test_derive_256_bit_salt_and_key(self):
        """
        ...
        salt, key_1 = derive_256_bit_salt_and_key(password)
        ph = PasswordHasher()
    ...
    def test_derive_256_bit_salt_and_key_given_valid_salt(self):
        """
        ...
        original_salt, original_key = derive_256_bit_salt_and_key(password=password)

        # Use the function with the salt
        salt_1, key_1 = derive_256_bit_salt_and_key(password=password, salt=original_salt)

        # Returned salt and key should be the same
        self.assertEquals(original_salt, salt_1)
        self.assertEquals(original_key, key_1)

    def test_derive_256_bit_salt_and_key_given_invalid_salt(self):
        """
        ...
        try:
            derive_256_bit_salt_and_key(password=password, salt=salt)
    ...
    def test_encrypt_aes_256_gcm_key_incorrect_length(self):
        """
        Encrypting should fail when given key is not 256-bit (32 bytes long).
        """
        try:
    ...
    def test_decrypt_aes_256_gcm_wrong_ciphertext(self):
        ...
        plaintext = 'plaintext'
        salt, key_1 = derive_256_bit_salt_and_key('password')
        salt, key_2 = derive_256_bit_salt_and_key('password')
